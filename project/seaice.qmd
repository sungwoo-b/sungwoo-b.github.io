...
This project utilizes **Remote Sensing** data from NOAA Polar Watch, demonstrating the application of **projection** techniques and the integration of Animal Telemetry data.
...
이 프로젝트는 NOAA Polar Watch의 원격 감지(Remote Sensing) 데이터를 활용하여 평면지도에 적용할 수 있는 투영법(projection)과 동물 위치 추적(Animal Telemetry) 데이터를 통합한 분석을 소개합니다.
원격 감지 데이터 중에는 널리 알려진 해빙에 관한 데이터도 포함되어 있습니다. 이 프로젝트에서는 남극의 해빙 농도 데이터를 가지고 평면 지도에 투영법에 시각화 하였으며 여기에 Adelie 펭귄 추적 데이터를 추가하여 어떠한 분석이 가능한지를 보여주었습니다.


# 해빙 농도 테이터 (Sea Ice Concentration)

해빙 농도 데이터는 NOAA Polar Watch ERDDAP 데이터 서버에서 얻었으며 이 프로젝트에서는 NetCDF 형식을 사용했습니다. 
이 데이터는 대표적으로 시간, 좌표, 해빙 농도 등 여러 정보를 포함하고 있습니다.
...
그러나 아래에서 소개하는 두 가지 방법을 사용하면 해빙 데이터 뿐 아니라 다른 지리공간 데이터도 평면 지도에 시각화할 수 있습니다.

**I. Polar Stereographic Projection**

Polar Stereographic Projection은 지구의 극 지역을 중심으로 하는 투영법입니다.
이 프로젝트에서는 아래와 같이 남극을 중심으로 해빙 농도 데이터를 시각화 하였습니다.
...
위 지도에서 파란색 영역은 해빙 농도(Sea Ice Concentration)를 나타내며 지도 아래에 표시된 컬러바는 해빙 농도 값을 보여줍니다.

**II. EPSG Code 3412**

처음으로 소개한 방법은 극지역을 중심으로 한 투영방법이지만 또 다른 투영법으로 위치별 EPSG 코드를 사용하여 직접 투영할 위치를 적용하는 것입니다. 남극의 EPSG 코드는 3412이며 아래 지도는 이 코드를 사용하여 투영법을 적용한 결과를 보여줍니다.
...
데이터를 보다 잘 이해할 수 있도록 위에 분포를 나타내었으며 이를 통해 펭귄의 카테고리별 추가 분석이 가능합니다.
...
ADPE24는 2003년 1월부터 3월까지의 데이터가 존재하며 위에서 보이듯 섬에서 출발하여 해빙 농도가 높은 지역으로 이동한 것을 볼 수 있습니다.
해빙 농도는 월별로 변하며 ADPE24가 이러한 변화를 따라가는 것이 나타납니다.
비록 한 개체의 사례이지만 해빙과 펭귄 이동 패턴 사이에 관계가 있을 수 있음을 시사합니다.

이 프로젝트는 지도 투영법을 사용하여 원격 탐지(Remote Sensing) 데이터를 시각화하고 이를 다른 데이터와 통합하여 비교 분석할 수 있음을 보여줍니다.
또한 여러 가지 투영법이 존재하며 이를 활용하면 다양한 지리공간 데이터를 효과적으로 표현할 수 있습니다.
